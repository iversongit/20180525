...
function updateHouseData(action) {
    ...
    var old_start_date = location.search.split('&')[2].split('=')[1];
    var old_end_date = location.search.split('&')[3].split('=')[1];
    var path = location.search.replace('sd='+old_start_date,'sd='+startDate);
    path = path.replace('ed='+ old_end_date,'ed='+ endDate);
    location.href = path;
}
...
$(document).ready(function(){
    ...
    var sortName = queryData["sk"];
    if(sortName == 'new'){
        // alert(sortName);
        sortName = '最新上线'
    }else if(sortName == 'booking'){
        sortName = '入住最多'
    }else if(sortName == 'price-inc'){
        sortName = '价格 低-高'
    }else if(sortName == 'price-des'){
        sortName = '价格 高-低'
    }
    $(".filter-title-bar>.filter-title").eq(2).children("span").eq(0).html(sortName);
    ...
    $(".filter-item-bar>.filter-area").on("click", "li", function(e) {
        if (!$(this).hasClass("active")) {
            ...
            var old_area_id = location.search.split('&')[0].split('=')[1];
            var new_area_id = $(this).attr('area-id');
            var old_area_name = location.search.split('&')[1].split('=')[1];
            var new_area_name = $(this).html();
            // alert(new_area_name);
            var path = location.search.replace('aid='+old_area_id,'aid='+new_area_id);
            path = path.replace('aname='+old_area_name,'aname='+new_area_name);
            // alert(path);
            location.href = path;
        } else {
    ...
    $(".filter-item-bar>.filter-sort").on("click", "li", function(e) {
        ...
        var sort_key = $(this).attr('sort-key');
            // var path = location.search + '&sort_key=' + sort_key;
        // alert(location.search.split('&sk=')[0]);
...
$(document).ready(function () {
    ...
    $.get('/house/allsearch/'+path,function (data) {
        if(data.code==200){
            ...
            $('.house-list').append(house_info_html);
